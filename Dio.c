...
#include "Det.h"        /*check version bayny w baynoh*/
...
uint8 Dio_Status = DIO_NOT_INITIALIZED;/*INITIALIZED by call init not init det error */
...
void Dio_Init(const Dio_ConfigType * ConfigPtr)
{
	...
	if (NULL_PTR == ConfigPtr)/* if null det error*/
	{
		Det_ReportError(DIO_MODULE_ID, DIO_INSTANCE_ID, DIO_INIT_SID,
		     DIO_E_PARAM_CONFIG);
	}
	else/*no error*/
#endif  /*in case i delete det error clean if with else*/
	{
		/*
		 * Set the module state to initialized and point to the PB configuration structure using a global pointer.
		 * This global pointer is global to be used by other functions to read the PB configuration structures
		 */
		Dio_Status       = DIO_INITIALIZED;     /*naqleb qemet al variable*/
		Dio_PortChannels = ConfigPtr->Channels; /* address of the first Channels structure --> Channels[0] (point on configration wich is structure increament by structure) */
	}
}
...
void Dio_WriteChannel(Dio_ChannelType ChannelId, Dio_LevelType Level)
{
...
#if (DIO_DEV_ERROR_DETECT == STD_ON)/*I will check on 2 error*/
	...
	if (DIO_NOT_INITIALIZED == Dio_Status)
	{
		...
		error = TRUE;/*7asal 7aga*/
	}
	else
	{
		/* No Action Required */
	}
	/* Check if the used channel is within the valid range 'configered rang'*/
        /*         configered rang   <= number gayaly(1,2)  */
	if (DIO_CONFIGURED_CHANNLES <= ChannelId) /*why not if elseif it is wrong because the berpouse of module report as much as error bigest no. of error */
	{
	...
	if(FALSE == error)
	{
		/* Point to the correct PORT register according to the Port Id stored in the Port_Num member */
                /*how to write
                  u will git indx by it u access structure & get port number and pin number
                                       [index'0,1'] Port_Num(0=A,1=B,2=C,3=D,4=E,5=F)
                25rog men switch case pointer to rehister 'one if sex ports'
                bhy al7arakh 2sta5damet if els mrah wa7da bs bdal ma 7tha da5l kl case */
		...
		if(Level == STD_HIGH)/*bdy 2ktob 1*/
		{
			/* Write Logic High */
                        /*     (acces rigester, wich pin[index].port number)*/
			SET_BIT(*Port_Ptr,Dio_PortChannels[ChannelId].Ch_Num);
		}
		else if(Level == STD_LOW)/*bdy 2ktob 0*/
		{
			/* Write Logic Low */
...
Dio_LevelType Dio_ReadChannel(Dio_ChannelType ChannelId)
{
	...
	if(FALSE == error)
	{
		...
		/* Read the required channel */
                /* get from swich case pointer on register [santex array(0,1)] . no.port(0=A,1=b,2=C,3=D,4=E,5=F)
                   we use pointer for ports for just use if else once to point to evry case*/
